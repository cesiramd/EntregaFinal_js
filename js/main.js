...
let isPressed = false;
      ...
      $( window ).on( 'mousemove', function( e ){
          ...
          if (isPressed){
            //  let geometry = new THREE.SphereGeometry( ( Math.abs( mouse.x) * 0.3) + 0.2, ( Math.abs( mouse.y) * 0.3) + 0.2 , 0.5);
            let geometry = new THREE.BoxGeometry(0.08,0.08,0.8);
              let color = new THREE.Color("rgb(227,219,68)");
              let material = new THREE.MeshLambertMaterial({ color: color });
              let circulo = new THREE.Mesh(geometry, material) ;

               //let geometry2 = new THREE.SphereGeometry( ( Math.abs( mouse.x) * 0.3) + 0.1, ( Math.abs( mouse.y) * 0.3) + 0.1 , 0.6 );
            let geometry2 = new THREE.BoxGeometry(0.06,0.06,1);
              let color2 = new THREE.Color("rgb(255,242,0)");
